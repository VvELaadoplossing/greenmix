...
async function fetchAndStore(env, source, opts = {}) {
  ...
  const fromDate = opts.fromDate ? ymd(opts.fromDate) : ymd(now - daysBack * day);
  const toDate = opts.toDate ? ymd(opts.toDate) : ymd(now + daysFwd * day);
...
async function queryGreenmix(env, source, params) {
  return aggregateGreen(await queryRows(env, source, params));
}

// Forecast vs realized, joined on (type_id, ts_utc) over the requested window.
// Only timestamps present in BOTH tables are compared. Reports per-type volume
// error and the error in the overall green share. Note: this is only honest for
// data the cron captured prospectively — a backfilled forecast is recalculated
// by Ned and would understate the real day-ahead error.
async function queryCompare(env, params) {
  const fRows = await queryRows(env, "forecast", params);
  const rRows = await queryRows(env, "realized", params);

  const rByKey = new Map(rRows.map((x) => [`${x.type_id}|${x.ts_utc}`, x]));
  const acc = new Map();
  for (const f of fRows) {
    const r = rByKey.get(`${f.type_id}|${f.ts_utc}`);
    if (!r || f.volume_kwh == null || r.volume_kwh == null) continue;
    const a = acc.get(f.type_id) || { type_id: f.type_id, type_name: f.type_name, n: 0, sumAbs: 0, sumBias: 0, sumReal: 0 };
    const diff = r.volume_kwh - f.volume_kwh; // realized - forecast
    a.n++; a.sumAbs += Math.abs(diff); a.sumBias += diff; a.sumReal += r.volume_kwh;
    acc.set(f.type_id, a);
  }
  const per_type = [...acc.values()]
    .map((a) => ({
      type_id: a.type_id,
      type_name: a.type_name,
      n: a.n,
      mae_kwh: a.n ? a.sumAbs / a.n : null,          // mean absolute error
      mean_bias_kwh: a.n ? a.sumBias / a.n : null,   // + => realized exceeded forecast
      mape_pct: a.sumReal ? (100 * a.sumAbs) / a.sumReal : null,
    }))
    .sort((x, y) => x.type_id - y.type_id);

  // error in the overall green share, per matched timestamp
  const fShareByTs = new Map(aggregateGreen(fRows).map((o) => [o.ts_utc, o.green_share]));
  let gN = 0, gSumAbs = 0, gSumF = 0, gSumR = 0;
  for (const o of aggregateGreen(rRows)) {
    const fShare = fShareByTs.get(o.ts_utc);
    if (fShare == null || o.green_share == null) continue;
    gN++; gSumAbs += Math.abs(o.green_share - fShare); gSumF += fShare; gSumR += o.green_share;
  }
  const green_share = {
    n: gN,
    forecast_avg: gN ? gSumF / gN : null,
    realized_avg: gN ? gSumR / gN : null,
    mae: gN ? gSumAbs / gN : null,                   // avg abs error in green share (0..1)
  };

  return { window: { from: params.get("from") || null, to: params.get("to") || null }, per_type, green_share };
}
...
const HELP = `Greenmix (Ned.nl) — read-only API
  ...
  GET /api/refresh?token=...   run the collector now (needs REFRESH_TOKEN)
                               ...
                               (overrides days_back/days_fwd; for probes/backfill)
                               ...
                               separately (each is its own invocation), and keep
                               backfill ranges to ~1 month per call.
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (p === "/api/refresh") {
        ...
        if (q.get("from")) opts.fromDate = q.get("from"); // YYYY-MM-DD; overrides days_back
